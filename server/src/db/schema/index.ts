import { pgTable, index, foreignKey, check, uuid, varchar, text, timestamp, integer, unique, boolean, bigint, jsonb, uniqueIndex, bigserial, primaryKey, pgEnum } from "drizzle-orm/pg-core"
...
export const rooms = pgTable("rooms", {
	...
	check("group_dm_requires_owner", sql`((type = 'GROUP_DM'::text) AND (owner_id IS NOT NULL)) OR ((type <> 'GROUP_DM'::text) AND (owner_id IS NULL))`),
	check("server_channel_requires_server", sql`((type = ANY (ARRAY['SERVER_TEXT_CHANNEL'::text, 'SERVER_VOICE_CHANNEL'::text])) AND (server_id IS NOT NULL)) OR ((type = ANY (ARRAY['DM'::text, 'GROUP_DM'::text])) AND (server_id IS NULL))`),
	check("voice_channel_members_check", sql`((type = 'SERVER_VOICE_CHANNEL'::text) AND (total_members IS NOT NULL) AND (max_members IS NOT NULL)) OR (type <> 'SERVER_VOICE_CHANNEL'::text)`),
	check("rooms_type_check", sql`type = ANY (ARRAY['DM'::text, 'GROUP_DM'::text, 'SERVER_TEXT_CHANNEL'::text, 'SERVER_VOICE_CHANNEL'::text])`),
]);
...
export const messages = pgTable("messages", {
	...
	id: bigint({ mode: "number" }).primaryKey().generatedAlwaysAsIdentity({ name: "messages_id_seq", startWith: 1, increment: 1, minValue: 1, cache: 1 }),
	...
	index("idx_messages_channel").using("btree", table.roomId.asc().nullsLast().op("timestamptz_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	foreignKey({
...
export const passwordHistory = pgTable("password_history", {
	...
	index("password_history_user_created_idx").using("btree", table.userId.asc().nullsLast().op("timestamptz_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	uniqueIndex("password_history_user_hash_unique").using("btree", table.userId.asc().nullsLast().op("text_ops"), table.passwordHash.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
